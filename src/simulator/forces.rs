...
pub fn buoyancy(altitude: f32, atmo: atmosphere::Atmosphere, lift_gas: gas::GasVolume) -> f32 {
    ...
    if v > 0.0 {
        let rho_atmo = atmo.density(altitude);
...
pub fn drag<T: SolidBody>(altitude: f32, atmo: atmosphere::Atmosphere, velocity: f32, body: T) -> f32 {
    // Force (N) due to drag against the balloon
    let direction = -f32::copysign(1.0, velocity);
    direction * body.drag_coeff() / 2.0 * atmo.density(altitude) * f32::powf(velocity, 2.0) * body.drag_area()
}

pub fn gross_lift(altitude: f32, atmo: atmosphere::Atmosphere, lift_gas: gas::GasVolume) -> f32 {
    // [kg]
    let rho_atmo = atmo.density(altitude);
    let rho_lift = lift_gas.density();
...
pub fn free_lift(altitude: f32, atmo: atmosphere::Atmosphere, lift_gas: gas::GasVolume, total_dry_mass: f32) -> f32 {
    // [kg]
