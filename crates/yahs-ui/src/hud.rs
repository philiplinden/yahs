...
fn update_hud(
    ...
    if let Ok(mut kinematics_text) = query.get_single_mut() {
        ...
        for (name, transform, weight, buoyancy, drag, net_force, velocity, gas) in balloons.iter() {
...
fn update_settings_text(
    ...
    if let Ok(mut settings_text) = query.get_single_mut() {
        ...
        text.push_str(&format!("Toggle Real Time: {:?}\n", key_bindings.time_controls.toggle_real_time));
